...
async function search(query: string) {
  ...
  await user.type(screen.getByLabelText('포인트명 또는 테스트 지역'), query)
...
describe('Search-first UI — Must requirements', () => {
  it('distinguishes duplicate candidates and waits for explicit selection', async () => {
    ...
    expect(screen.queryByText('현재 합성 데이터가 모두 도착했습니다')).not.toBeInTheDocument()
  })
  it.each([
    ['미지원', '미지원 포인트'], ['모호', '포인트 식별이 모호합니다'], ['카탈로그오류', '카탈로그를 사용할 수 없습니다'],
  ])('shows %s without a brief', async (query, message) => { await search(query); expect(screen.getByText(message)).toBeVisible(); expect(screen.queryByText(/선택한 합성 포인트/)).not.toBeInTheDocument() })
  it('shows partial siblings, all five text+icon states, evidence and exact focus return', async () => {
    ...
    expect(screen.getByText('일부 합성 정보만 확인됐습니다')).toBeVisible()
    for (const state of ['확인됨', '오래됨', '확인 필요', '수집 실패', '정보충돌']) expect(screen.getByText(state)).toBeVisible()
  ...
  it('retries a failed slot without hiding successful siblings', async () => {
    ...
    expect(screen.getByText('합성 확인 값')).toBeVisible()
  ...
  it('never presents absence as permission, safety or recommendation', async () => {
    ...
    expect(screen.getByText('현재 확인 가능한 합성 데이터가 없습니다')).toBeVisible()
