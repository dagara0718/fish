...
export function EvidencePanel({ record, onClose }: { record: InformationRecord; onClose: () => void }) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <div ref={panelRef} className="evidence-panel" role="dialog" aria-modal="true" aria-labelledby="evidence-title">
      <header className="dialog-header"><div><p className="identity-kicker">데이터 근거 · DEMO</p><h2 id="evidence-title">{record.label}</h2></div><button type="button" className="secondary-button" onClick={onClose} aria-label="근거 패널 닫기">닫기</button></header>
